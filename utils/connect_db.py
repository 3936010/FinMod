...
class ConnectDB:
    def __init__(self):
        ...
        if self.db_user and self.db_password:
            print("DB_USER and DB_PASSWORD are set, using MySQL.")
            ...
            print("Warning: DB credentials not set. Using local SQLite database (local_data.db).")
            # Use a local SQLite database file
            self.conn_string = 'sqlite:///local_data.db'

        self.engine = create_engine(self.conn_string)

        # This is for speeding up the insertion into the database schema
        # Only apply fast_executemany for drivers that support it (mostly purely ODBC/Azure, but kept for MySQL if previously working)
        # SQLite cursors do not support fast_executemany
        if not self.conn_string.startswith('sqlite'):
