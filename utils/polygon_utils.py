...
from utils.db_operations import DataOperations # Added import
...
def prepare_news_for_llm(news_list: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    Args:
        news_list: List of dictionaries or Pydantic models containing news data.
    ...
    for news_item in news_list:
        ...
        if hasattr(news_item, 'model_dump'):
            cleaned_item = news_item.model_dump()
        else:
...
def parse_news_by_date(news_list: List[Dict[str, Any]]):
    """
    Parse news (list of dicts or Pydantic models) by date and return a dictionary with date as key and news as value.
    """
    news_dict = {}
    for news in news_list:
        try:
            # Handle both Pydantic models and dicts
            if hasattr(news, 'published_utc'):
                published_utc = news.published_utc
            else:
                published_utc = news.get('published_utc')
            
            # Parse the ISO timestamp string
            date_str = pd.to_datetime(published_utc).strftime('%Y-%m-%d')
            if date_str not in news_dict:
                news_dict[date_str] = []
            news_dict[date_str].append(news)
        except Exception as e:
            # Get id from Pydantic model or dict
            news_id = getattr(news, 'id', None) or (news.get('id', 'N/A') if isinstance(news, dict) else 'N/A')
            print(f"Could not parse date for news item: {news_id}, error: {e}")
